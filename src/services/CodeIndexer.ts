...
export class CodeIndexer {
  ...
  async indexWorkspace(): Promise<void> {
    ...
    if (folders.length === 0) {
      ...
      throw new Error(
        "No workspace folders found. Please open a workspace first."
      )
    }

    console.log(`Starting to index ${folders.length} workspace folders`)
    ...
    for (const folder of folders) {
      try {
        ...
        console.log(
          `Found ${uris.length} files to index in ${folder.uri.fsPath}`
        )

        if (uris.length === 0) {
          console.warn(
            `No code files found in ${folder.uri.fsPath}. Make sure you have TypeScript, JavaScript, Python, Java, or Go files.`
          )
        }

        await Promise.all(uris.map((u) => this.indexFile(u)))
        console.log(`Indexed ${uris.length} files from ${folder.uri.fsPath}`)
      } catch (error) {
    ...
    console.log(`Indexing complete. Total files indexed: ${this.index.size}`)

    if (this.index.size === 0) {
      throw new Error(
        "No files were indexed. Check if your workspace contains supported file types (.ts, .js, .py, .java, .go)"
      )
    }
  }
  ...
  async indexFile(uri: vscode.Uri): Promise<void> {
    try {
      ...
      console.log(
        `Indexed file: ${metadata.path} (${metadata.fileType}, ${metadata.functions.length} functions, ${metadata.classes.length} classes)`
      )
    } catch (error) {
  ...
  search(query: string, maxResults: number = 10): ContextResult[] {
    ...
    console.log(`Searching for query: "${query}" with keywords:`, keywords)
    console.log(`Index contains ${this.index.size} files`)
  ...
  async findRelevantContext(
    ...
    const keywords = this.extractKeywords(query)
    const ticketKeywords = ticketData
      ? this.extractKeywords(ticketData.summary + " " + ticketData.description)
      : []
    ...
    return uniqueResults.sort((a, b) => b.relevance - a.relevance).slice(0, 15)
  }
  ...
  private calculateRelevance(
    ...
    for (const keyword of keywords) {
      const tokenMatches = metadata.tokens.filter(
    ...
    if (
      ...
      (metadata.path.includes("api") || metadata.path.includes("service"))
    ) {
      score += 3
    }
  ...
  private findTypeMatches(query: string): ContextResult[] {
    ...
    for (const [uri, metadata] of this.index) {
      ...
      if (
        ...
        metadata.fileType === "TypeScript" &&
        (metadata.path.includes("component") ||
          metadata.classes.some((cls) => cls.includes("Component")))
      ) {
        relevance += 5
      ...
      if (
        ...
        metadata.fileType === "TypeScript" &&
        (metadata.path.includes("service") ||
          metadata.classes.some((cls) => cls.includes("Service")))
      ) {
        relevance += 5
      ...
      if (
        ...
        (metadata.path.includes("api") || metadata.path.includes("endpoint"))
      ) {
        relevance += 5
        matches.push("api")
      }
